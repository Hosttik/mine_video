...
class DiagnosticsTests(unittest.TestCase):
    def test_bundle_contains_runtime_state_but_not_secrets(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            (job_dir / "obs.log").write_text("connected with obs-secret", encoding="utf-8")
            secrets = (config.api_token, config.rcon_password, config.obs_password, config.guard_token)
