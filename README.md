...
Нужны Python 3.11+, Docker Compose, Minecraft Java с лицензией, Prism Launcher, Java 21, OBS 28+ и FFmpeg с libass/libx264. Клиент, OBS и Python-worker запускаются в одной графической пользовательской сессии; только Paper работает в Docker. Обычный сервер без графического клиента не может записывать экран Minecraft.
...
5. Проверить подключения и запустить сервис:
...
`start` поднимает API и worker. Paper, Prism и OBS запускаются при первом задании, если ещё не работают. `doctor --launch` делает это заранее. В браузере API-документация доступна по `http://127.0.0.1:8000/docs`; в Authorize вставить `MV_API_TOKEN` из `.env`.

Для первого прогона полезно видеть окно Minecraft и предпросмотр OBS: проверка heartbeat и изменения кадров выявляет технический сбой, но не оценивает привлекательность кадра или качество звукового микса.
...
| --- | --- | --- |
| GET | `/health` | Доступность API и свежесть worker heartbeat |
...
| GET | `/jobs/{id}/artifacts/{name}` | Скачать готовый артефакт |
...
Одна очередь обслуживает одну записывающую машину. API можно запустить отдельно (`mine-video serve`), worker — отдельно (`mine-video worker`). Все worker этой студии должны использовать один `data_dir`; файловый lock предотвращает одновременный захват. Для нескольких машин нужны отдельные студии и распределитель заданий.
...
Тесты проверяют конкурентные claims, idempotency, отмену, восстановление, ограничение сцен, защиту субтитров от ASS-вставок и настоящий FFmpeg-экспорт со звуком в обоих форматах. Внешние игровые/OBS-адаптеры в тестах worker заменены управляемыми заглушками. Синтетический видеопаттерн используется только в тесте кодировщика; в рабочем конвейере такого режима нет.

CI устанавливает Python-зависимости, запускает тесты и собирает Fabric JAR. Это не заменяет сквозную запись с лицензированным графическим клиентом. Текущая фактическая проверка описана в [VALIDATION.md](docs/VALIDATION.md).
